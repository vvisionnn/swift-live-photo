...
public final class LivePhoto {
	...
	private func generate(
		...
		guard let keyPhotoURL = _keyPhotoURL, let pairedImageURL = addAssetID(
			assetIdentifier,
			toImage: keyPhotoURL,
...
extension AVAsset {
	fileprivate func countFrames(exact: Bool) -> Int {
		var frameCount = 0
	...
	fileprivate func stillImageTime() -> CMTime? {
	...
	fileprivate func makeStillImageTimeRange(percent: Float, inFrameCount: Int = 0) -> CMTimeRange {
		var time = duration
	...
	fileprivate func getAssetFrame(percent: Float) -> UIImage? {
